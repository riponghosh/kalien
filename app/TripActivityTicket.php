...
class TripActivityTicket extends Model
{
    ...
    public function Trip_activity(){
        return $this->belongsTo('App\Product','trip_activity_id','id')->where('pdt_type', ProductTypeEnum::GROUP_ACTIVITY);
    }

    public function fix_time_ranges(){
        return $this->hasOne('App\TripActivityTicket\FixTimeRange', 'trip_activity_ticket_id', 'id');
    }

    public function disable_dates(){
        return $this->hasMany('App\TripActivityTicket\DisableDate', 'activity_ticket_id', 'id');
    }

    public function disable_weeks(){
        return $this->hasMany('App\TripActivityTicket\DisableWeek','activity_ticket_id', 'id');
    }
