...
@section('header')
    <div class="group-activity-header" style="background-image: url({{Storage::url($trip_activity['trip_gallery_pic'])}})">
                ...
                <div class="organiserInfo-section">
                    ...
                    <div class="organiserInfo-section-body">
                        ...
                        <div>
                            ...
                            <p class="display: block;padding-bottom: 5px;"><span class="m-r-10">人數({{count($group_activity->applicants)}}/{{$limit_joiner}})</span>
                                @if($group_activity->is_achieved == true)
                                    <span class="label label-success">成團</span>
                                @elseif($group_activity->foridden_reason == 2)
                                    <span class="label label-default">未達成團人數</span>
                                @elseif($group_activity->foridden_reason == 4)
                                    <span class="label label-danger">成團失敗</span>
                                @elseif($group_activity->foridden_reason == 5)
                            ...
                            @if(count($group_activity->applicants) > 0)
                                    @foreach($group_activity->applicants as $applicant)
                                        ...
                                        <div class="d-inline-block m-r-5 m-b-5" data-toggle="tooltip" title="{{$applicant['user']['name']}}" style="width: 30px;height: 30px;">
                                            <img class="img-circle" src="{{$applicant_icons}}" data-toggle="tooltip" style="width: 100%; height: 100%;">
                                        </div>
                                    @endforeach
                            @endif
                        ...
                        <div class="row">
                            <div class="col-sm-6">
                                <div class="font-c-pneko-red" style="font-size: 22px;">
                                    NT$ {{$group_activity->trip_activity_ticket->amount}}
                                </div>
